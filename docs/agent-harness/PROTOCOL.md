...
type LiveEvent =
  ...
  | { t: "event"; kind: "input-ready"; flushed: number; dropped: number; ts: number }
...
- `input-ready`: **The TUI can now receive input.** Emitted exactly once per process, at the moment the React input bridge registers its command handler. The transport starts accepting JSONL commands ~120 ms after process start (and on Windows the handshake that unblocks the parent is written before that), but nothing consumed them until the bridge mounted — measured 445–745 ms later on a warm box — and every `type`/`press` sent inside that window was discarded by an empty handler array with no error and no event. Commands are now buffered across the window (bounded at 256; overflow rejects the newest, logs to stderr, and is counted). `flushed` is how many buffered commands were replayed into the bridge; `dropped` is how many were lost to overflow (normally `0` — a non-zero value means input WAS lost and the run is not trustworthy). Gate on `wait_for({event:"input-ready"})`, **not** `wait_for({idle:true})`: the event condition is replay-safe (the driver scans its buffered event ring, so late subscription still resolves), whereas `idle` is captured-start based and routinely resolves on an empty pre-mount frame — measured `ok` 82 ms after start on a frame with no composer in it.
...
- `rate-limit-wait`: **A call was deliberately HELD to stay inside a provider limit the catalog declares.** Emitted by the request pacer (`src/providers/rate-limiter.ts`) through the metered gate, only when a wait was actually incurred — a call that fits the budget emits nothing. `provider` is the account whose budget bound, `modelId` the model the held call was going to, `stage` its pipeline stage, `limitKind` which budget bound (`requests-per-minute` | `concurrency`), `limit` the declared ceiling verbatim from `catalog.json`, and `waitMs` how long the call was held. Distinct from `stream-retry` on purpose, for the same reason `model-fallback` is: `stream-retry` means an error already happened and the SAME call is being re-attempted, carrying `attempt`/`maxAttempts`/`errorName`/`errorMessage`; a pacing wait is the opposite — no error occurred, no attempt was consumed, and the request has not been sent yet. Folding them would mean inventing an error name and an attempt number for a healthy call and would corrupt any retry metric built on `stream-retry`. **A driver needs this**: without it a paced call is indistinguishable from a hung one. Motivating measurement (2026-09-06): nine StepFun models declared `requests_per_minute: 10` that no production code read, a `/ideal` run dispatched 15 requests in the 60 s before each rate-limit error, and the third such error exhausted the retry loop and ended a ~20-minute run.
