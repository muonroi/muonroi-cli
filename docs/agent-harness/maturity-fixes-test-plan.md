...
- **Why:** the `fileTracker.writtenPaths()` empty-ledger guard in `git_commit` (`src/tools/registry.ts`, message built by `emptyLedgerRefusalMessage` in `src/tools/commit-ledger-refusal.ts`) + `isExcludedPath` (`src/orchestrator/auto-commit.ts`). Regression → nothing-files noise, or a `.env` leak. Referenced by SYMBOL, not line: the two line numbers this bullet used to carry (`registry.ts:465-471`, `auto-commit.ts:201`) had both drifted to `:867` and `:263` by the time anyone checked.
...
- **Acceptance:** ☐ A: stdout names the LEDGER SCOPE (mentions `write_file`/`edit_file`), reports the repo's own uncommitted state, and names the `bash` + `git commit` route — pin the shape, NOT the prose, so a copy edit does not fail the gate (the old criterion pinned the verbatim string `Nothing to commit — you have not created or edited any file`, which `31b24db7` replaced) ☐ A: `rev-list --count HEAD` is 0/errors ☐ B: `show --name-only HEAD` contains `config.ts` ☐ B: does NOT contain `debug.log` ☐ B: `status --porcelain` shows `debug.log` as `??` ☐ B: message ends `Coding by - Muonroi-CLI`
