...
**What goes wrong:**
The 3-tier router routes ~8% of calls to Ollama on the VPS (`experience.muonroi.com`). The VPS goes down — disk full, OOM, qwen2.5-coder process crashed, network partition. The router still tries warm-path. Every call times out at 10s. The product feels broken. The fallback to cold-path was never written because warm-path "always works." Or worse, the router silently downgrades EVERY call to cold-path — SiliconFlow gets hammered, latency spikes, costs jump because cold-path is paid.
