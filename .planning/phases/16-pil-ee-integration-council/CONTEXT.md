...
- **EE thin-client is HEALTHY** on VPS:
  - `curl -H "Authorization: Bearer <readToken>" https://experience.muonroi.com/health` → 200
...
2. Run with VPS unreachable (block `experience.muonroi.com`) → council completes within latency budget, skips warnings, queues outcome offline; chat UI shows EE health indicator turn red but no crash
