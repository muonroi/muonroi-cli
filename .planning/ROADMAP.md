...
- v1.4 Architecture Quality (Phase 12.1, 12.2, 12.3, 12.4, 12.5, 12.6) - active
...
- [x] **Phase 12.5: Extract BatchTurnRunner** — SHIPPED 2026-05-20 (12.5-01 add BatchTurnRunner module `6276269` + 12.5-02 delegate processMessageBatchTurn `6805218` + 12.5-03 sweep + tests). orchestrator.ts: 2848 → 2641 LOC (−207). BatchTurnRunner: 457 LOC (~22 callback DI surface) owns the batch-API turn loop (per-turn compaction with overflow-relax, MCP toolset assembly, polling, executeBatchToolCall delegation, transient retry with harness `stream-retry` emit, MCP teardown). Callback names aligned with MessageProcessorDeps (`getCompactionSettings`, `compactForContext`, `postTurnCompact`, `recordUsage`, `appendCompletedTurn`, `discardAbortedTurn`, `getCompactedThisTurn` / `setCompactedThisTurn`) so a future `TurnRunnerDepsBase` hoist is mechanical. 5 unit tests cover DI invariants (constructor, abort path, compactedThisTurn reset, recordUsage guard, mutable messages reference); full unit suite 2698 passes; F1/G1/B3/B4/C3 cost-leak harness specs green.
- [x] **Phase 12.6: Hoist TurnRunnerDepsBase** — SHIPPED 2026-05-20 — pure type-level refactor. New `src/orchestrator/turn-runner-deps.ts` exports `TurnRunnerDepsBase` interface holding the 19 properties that are identical across `MessageProcessorDeps` and `BatchTurnRunnerDeps` (messages, bash, mode, maxToolRounds, schedules, sendTelegramFile, getCompactedThisTurn / setCompactedThisTurn, setLastProviderOptionsShape, getCompactionSettings, compactForContext, postTurnCompact, runTask, runDelegation, readDelegation, listDelegations, appendCompletedTurn, discardAbortedTurn, recordUsage). Both concrete deps interfaces now `extends TurnRunnerDepsBase`; orchestrator.ts call sites unchanged (object literal still satisfies the extended interfaces). Zero runtime change. tsc 0 errors; full unit suite 2717 pass / 10 skip / 2 todo; F1/G1/B3/B4/C3 harness cost-leak specs (17 tests) green.
