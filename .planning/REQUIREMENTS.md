...
- [ ] **FORK-01**: Fork `grok-cli` into `muonroi-cli` with first commit referencing `IDEA.md` and preserving `LICENSE-grok-cli` immutable. (Pitfall 15)
...
- [ ] **FORK-05**: Create `UPSTREAM_DEPS.md` listing every external dependency with release-feed pointer; add CI job running `bun outdated` weekly. (Pitfall 1)
- [ ] **FORK-06**: Create `DECISIONS.md` at repo root for locked architectural decisions; first entries cover license, storage paths, `.muonroi-flow/` naming, Bun pin. (Open Q1, Q2, Q3, Q5)
