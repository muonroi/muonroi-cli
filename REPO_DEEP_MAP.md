...
|---|---|---|---|---|---|
| `layer1-intent.ts` | Task/intent/style/depth/scope/language classification cascade | YES (Pass −1 model-first via `opts.llmFallback`; also legacy Pass 3 EE `/api/pil-context` and Pass 4 `LlmClassifyFn` fallback) | `layer1Intent(ctx, opts: Layer1Options): Promise<PipelineContext>` (`:674`); `scoreComplexity(input): ComplexityOutput` (`:134`); `scoreSufficiency` (`:96`); `isContinuationPhrase` (`:323`); `isStatusCheckQuestion` (`:359`); `hasActionableToolIntent` (`:507`); `isSocialPleasantry` (`:654`) | `ctx.raw`, `ctx.gsdPhase` | `taskType`, `domain`, `confidence`, `outputStyle`, `intentKind`, `deliverableKind`, `modelDepthTier`, `ecosystemScope`, `replyLanguage`, `_brainData`, `_intentTrace`, `gsdPhase`, `layers[]` |
| `llm-classify.ts` | Model-first classifier factory + result types; also sub-session routing heuristic | YES (`createLlmClassifier` streams a real chat completion) | `createLlmClassifier(factory, modelId): LlmClassifyFn` (`:291`); `LlmClassifyFn = (prompt, signal?) => Promise<LlmClassifyResult\|null>` (`:93`); `LlmClassifyResult` (`:49`, fields: taskType, outputStyle, confidence, intentKind, deliverableKind, depthTier, ecosystemScope, replyLanguage); `classifySubSessionActionHeuristic` (`:372`, deterministic regex) | n/a (pure function, called by layer1) | n/a — returns `LlmClassifyResult` consumed by layer1 |
