...
describe("withStreamRetry", () => {
  ...
  it("2. transient on attempt 1, success on 2 — factory called twice, onRetry fired once", async () => {
    ...
    const factory = vi.fn().mockRejectedValueOnce(makeTransientError()).mockResolvedValueOnce("second");
  ...
  it("4. transient on all 3 — final error thrown after maxAttempts", async () => {
    ...
    await expect(withStreamRetry(factory, { delay: fastDelay, onRetry: () => {} })).rejects.toBe(err);
  ...
  it("6. signal.aborted before first attempt — no factory call, AbortError thrown", async () => {
    ...
    await expect(withStreamRetry(factory, { delay: fastDelay, signal: controller.signal })).rejects.toMatchObject({
      name: "AbortError",
    });
  ...
  it("8. Retry-After header (seconds string) — uses that delay for THIS attempt", async () => {
    ...
    const factory = vi.fn().mockRejectedValueOnce(retryAfterErr).mockResolvedValueOnce("ok");
  ...
  it("9. jitter stays within ±jitter*delay range", async () => {
    ...
    for (let i = 0; i < 30; i++) {
      const delays: number[] = [];
      const factory = vi.fn().mockRejectedValueOnce(makeTransientError()).mockResolvedValueOnce("ok");
