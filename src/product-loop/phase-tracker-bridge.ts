...
import { fireAndForgetPhaseOutcome, type PhaseOutcomeKind, type FirePhaseOutcomeOpts } from "../ee/phase-outcome.js";
  ...
  if (snapshot) {
    fireAndForgetPhaseOutcome({
      ...
      evidence: {
        ...
        verifierResult: snapshot.verifyResult === "pass" ? { passed: 1, failed: 0 } 
                      : snapshot.verifyResult === "fail" ? { passed: 0, failed: 1 }
                      : undefined
      }
    }, opts);
  }
