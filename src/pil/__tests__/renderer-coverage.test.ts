...
describe("StructuredResponseView renderer coverage", () => {
  it("has a case for every RESPONSE_SCHEMAS key", async () => {
    ...
    // The codebase has TWO consumers of sr.taskType: the React renderer and the
    // text formatter (`_formatStructuredResponse`). Both must cover every key.
    const switchOffsets: number[] = [];
    let idx = 0;
    while ((idx = appSrc.indexOf("switch (sr.taskType)", idx)) !== -1) {
    ...
    for (const start of switchOffsets) {
      const body = appSrc.slice(start, start + 8000);
