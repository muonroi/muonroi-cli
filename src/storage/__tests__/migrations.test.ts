...
class InMemoryDatabase {
  ...
  private parseColumnDef(def: string): ColumnDef | null {
    ...
    if (defaultMatch) {
      ...
      defaultVal = isNaN(Number(raw)) ? raw : Number(raw);
    }
  ...
  private runQueryAll(sql: string, params: unknown[]): unknown[] {
    ...
    if (upper.startsWith('PRAGMA') && upper.includes('TABLE_INFO')) {
      ...
      const table = this.tables.get(tableName);
      if (!table) return [];
      return [...table.values()].map((col, i) => ({ cid: i, name: col.name, type: col.type, notnull: col.notNull ? 1 : 0, dflt_value: col.defaultVal, pk: 0 }));
    }
...
describe('DB migrations', () => {
  ...
  it('Test 2: DB at version 2 — migration v3 adds both columns without dropping existing ones', () => {
    ...
    raw.tables.set('usage_events', new Map([
  ...
  it('Test 4: row inserted without pil_active/enrichment_delta uses DEFAULT 0', () => {
    ...
    raw.rows.set('workspaces', [{ id: 'ws1', scope_key: '/tmp', canonical_path: '/tmp', display_name: 'test', last_seen_at: new Date().toISOString() }]);
    raw.rows.set('sessions', [{ id: 'sess1', workspace_id: 'ws1', model: 'claude', mode: 'safe', cwd_at_start: '/tmp', cwd_last: '/tmp', status: 'active', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }]);
  ...
  it('Test 5: row inserted with pilActive=true and enrichmentDelta=-120 stores 1 and -120', () => {
    ...
    raw.rows.set('workspaces', [{ id: 'ws2', scope_key: '/tmp2', canonical_path: '/tmp2', display_name: 'test2', last_seen_at: new Date().toISOString() }]);
    raw.rows.set('sessions', [{ id: 'sess2', workspace_id: 'ws2', model: 'claude', mode: 'safe', cwd_at_start: '/tmp2', cwd_last: '/tmp2', status: 'active', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }]);
    raw.rows.set('usage_events', []);
