/**
 * usd-meter.test.tsx -- tests for UsdMeter component.
 */
import { describe, expect, it } from "vitest";
import { UsdMeter } from "./usd-meter.js";

describe("UsdMeter", () => {
  it("renders session USD formatted to 2 decimals (no month)", () => {
    const el = UsdMeter({ session_usd: 3.5, current_pct: 40 }) as any;
    expect(el.props.children).toBe("session: $3.50");
  });

  it("renders white color when current_pct < 50", () => {
    const el = UsdMeter({ session_usd: 0, current_pct: 20 }) as any;
    expect(el.props.fg).toBe("white");
  });

  it("renders cyan color when current_pct >= 50 and < 80", () => {
    const el = UsdMeter({ session_usd: 0, current_pct: 55 }) as any;
    expect(el.props.fg).toBe("cyan");
  });

  it("renders yellow color when current_pct >= 80 and < 100", () => {
    const el = UsdMeter({ session_usd: 0, current_pct: 80 }) as any;
    expect(el.props.fg).toBe("yellow");
  });

  it("renders red color when current_pct >= 100", () => {
    const el = UsdMeter({ session_usd: 0, current_pct: 100 }) as any;
    expect(el.props.fg).toBe("red");
  });

  it("has data-testid usd-meter", () => {
    const el = UsdMeter({ session_usd: 0, current_pct: 0 }) as any;
    expect(el.props["data-testid"]).toBe("usd-meter");
  });
});
