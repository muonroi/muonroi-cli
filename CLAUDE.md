...
| `MUONROI_TOP_LEVEL_COMPACT_THRESHOLD_CHARS` | 10_000–1_500_000 | 30_000 | Phase B4 — single-turn tool-step history accumulation (forensics session f4d66be69216: ~5K input per step). Previously 200K, then 100K, then 30K so compaction kicks in by step 3-4 instead of step 10+. |
| `MUONROI_TOP_LEVEL_COMPACT_KEEP_LAST` | 1–30 | 3 | Phase B4 — lowered alongside threshold: 3 trailing turns kept verbatim, +2 on meta-analysis turns. |
